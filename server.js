...
function flattenBoard(board) {
  const arr = [];
  if (board.center) arr.push({ ...board.center, side: "center", placedOrder: board.center.placedOrder || 0 });
  for (const side of ["right","left","top","bottom"]) {
    for (const t of board.arms[side]) arr.push({ left:t.left, right:t.right, id:t.id, side, placedOrder:t.placedOrder || 0 });
  }
  return arr;
}
...
function placeOnBoard(room, tile, side) {
  ...
  // UI is RTL, so user "right" visually should map to logical left,
  // and user "left" visually should map to logical right.
  if (side === "right") side = "left";
  else if (side === "left") side = "right";

  if (!board.center) {
    board.center = { left: tile.left, right: tile.right, id: tile.id, placedOrder: room.placementCounter };
    return true;
  }

  if (!["right","left","top","bottom"].includes(side)) { room.placementCounter--; return false; }

  const ends = boardEnds(board);
  const openVal = ends[side];

  if (openVal === null || openVal === undefined) { room.placementCounter--; return false; }

  const oriented = orientedForSide(tile, openVal, side);
  if (!oriented) { room.placementCounter--; return false; }

  oriented.placedOrder = room.placementCounter;
...
io.on("connection", socket => {
  ...
  socket.on("createRoom", ({playerName,targetScore=350,teamMode=true}={}) => {
    const id = code();
    const room = {
      id, players:[], board:makeBoard(), boneyard:[], currentTurn:0, placementCounter:0, placementCounter:0, started:false,
  ...
  socket.on("findMatch", ({targetScore=350}={}) => {
    ...
    if (other && io.sockets.sockets.get(other.id)) {
      const id = code();
      const room = { id, players:[other, {...player,isHost:false}], board:makeBoard(), boneyard:[], currentTurn:0, placementCounter:0, placementCounter:0, started:false, winner:null, gameWinner:null, passes:0, mode:"matchmaking", roundNumber:0, targetScore:Number(targetScore)===450?450:350, teamMode:false, teams:{}, lastScore:null, penalty:null, penaltyAmount:5 };
      rooms[id] = room;
      assignTeams(room);
      for (const p of room.players) {
      ...
      setTimeout(() => {
        ...
        if (idx >= 0) {
          queue.splice(idx,1);
          const id = code();
          const room = { id, players:[player], board:makeBoard(), boneyard:[], currentTurn:0, placementCounter:0, placementCounter:0, started:false, winner:null, gameWinner:null, passes:0, mode:"botFallback", roundNumber:0, targetScore:Number(targetScore)===450?450:350, teamMode:false, teams:{}, lastScore:null, penalty:null, penaltyAmount:5 };
          rooms[id] = room;
          socket.join(id);
          socket.data.roomId = id;
          addBot(room);
